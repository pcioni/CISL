...
namespace IBM.Watson.DeveloperCloud.Widgets
{
	...
	public class NaturalLanguageClassifierWidget : Widget
    {
        ...
        {
            EventManagerWatson.Instance.RegisterEventReceiver("OnDebugCommand", OnDebugCommand);
        }
        private void OnDisable()
        {
            EventManagerWatson.Instance.UnregisterEventReceiver("OnDebugCommand", OnDebugCommand);
        }
        ...
        private void OnRecognize(Data data)
        {
            ...
            if (result.HasFinalResult())
            {
                ...
                EventManagerWatson.Instance.SendEvent("OnDebugMessage", string.Format("{0} ({1:0.00})", text, textConfidence));
                ...
                else
                {
                    ...
                    if (textConfidence > IgnoreWordConfidence)
                    {
                        ...
                        EventManagerWatson.Instance.SendEvent("OnClassifyFailure", result);
                    }
                }
            }
        }
        ...
        {
            EventManagerWatson.Instance.SendEvent("OnClassifyResult", result);
                ...
                if (!string.IsNullOrEmpty(result.top_class))
                {
                    if (result.topConfidence >= MinClassEventConfidence)
                    {
                        ...
                        if (!m_ClassEventMap.TryGetValue(result.top_class, out sendEvent))
                        {
                            ...
                            EventManagerWatson.Instance.SendEvent(result.top_class, result);
                        }
                        else
                            EventManagerWatson.Instance.SendEvent(sendEvent, result);
                    }
                    else
                    {
